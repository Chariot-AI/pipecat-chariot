...
Chariot serves lifelike voices for Indian languages and English.
...
class ChariotTTSSettings(TTSSettings):
    ...
    Parameters:
        ...
        model: Chariot TTS model. ``None`` uses the account default. Both this
            and ``speed`` are fixed for the life of a session, so changing
            either reconnects.
        speed: Speaking rate. Each voice supports a set of speeds under each
            model, commonly ``0.5``, ``0.75``, ``0.9``, ``1.1``, ``1.25`` and
            ``1.5``. ``GET /v1/voices`` lists what a given voice supports.
            ``None`` uses the voice as recorded.
    """
...
class ChariotTTSService(InterruptibleTTSService):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            model: Chariot TTS model, for example the multilingual model when
                it is available to your account. ``None`` uses the account
                default. ``settings.model`` takes precedence.
            speed: Speaking rate, one of the speeds this voice supports under
                this model, commonly ``0.5``, ``0.75``, ``0.9``, ``1.1``,
                ``1.25`` and ``1.5``. See ``GET /v1/voices``. ``None`` uses the
                voice as recorded. ``settings.speed`` takes precedence.
        ...
        # Every inherited settings field must be initialized, including the
        # ones Chariot does not support: the base class validates this at
        # startup and logs an error for any field left unset. Chariot takes no
        # model or language parameter, so both are explicitly None.
        ...
        # End of turn accounting. Pipecat calls run_tts once per sentence and a
        # long utterance is split into several flushed segments, so one turn
        # produces several audio.done events. The turn is over only when the
        # LLM turn has completed AND every segment we sent has come back.
        self._segments_in_flight = 0
    ...
    def _websocket_url(self) -> str:
        ...
        # Omitted rather than defaulted here, so the account default applies
        # and a new server side model or rate needs no change in this package.
        if self._settings.model is not None:
            params["model_type"] = self._settings.model
        if self._settings.speed is not None:
            params["speed"] = self._settings.speed
        return f"{self._base_url}/v1/tts/ws?{urlencode(params)}"
    ...
    async def _update_settings(self, delta: TTSSettings) -> dict:
        """Apply a settings delta, reconnecting when a session field changes.

        Voice, model and speed travel in the connection URL rather than in a
        message, so they are fixed for the life of a session. Changing any of
        them therefore means a new session.
        """
        changed = await super()._update_settings(delta)
        session_fields = {"voice", "model", "speed"}
        if (
            session_fields & set(changed)
            and self._websocket
    ...
    async def _finalize_audio_context(self, context_id: str | None) -> None:
        """Close the turn: emit the stop frame and release the audio context."""
        if not context_id:
            return
    ...
    async def on_turn_context_completed(self) -> None:
        """Finalize after the whole LLM turn, not after each flushed segment.

        Audio arrives on the receive task, so the turn may complete while
        segments are still generating. In that case the last audio.done
        finalizes instead.
        """
        self._turn_closing = True
        if self._segments_in_flight == 0:
            await self._finalize_audio_context(self.get_active_audio_context_id())
        await super().on_turn_context_completed()

    async def on_audio_context_interrupted(self, context_id: str) -> None:
        """Drop turn accounting on barge in; the socket is torn down anyway."""
        self._segments_in_flight = 0
        self._turn_closing = False
        ...
        try:
            async for message in self._get_websocket():
                ...
                if event_type == "audio.start":
                    ...
                    # One flushed SEGMENT finished, which is not the end of the
                    # turn: a multi sentence reply, or an utterance long enough
                    # to be segmented, produces several of these. Only close the
                    # turn once the LLM turn is done and nothing is still
                    # generating, otherwise the rest of the reply is dropped.
                    self._segments_in_flight = max(0, self._segments_in_flight - 1)
                    if self._turn_closing and self._segments_in_flight == 0:
                        await self._finalize_audio_context(context_id)
